...
void xlat_usb_hid_event(void)
{
    ...
    {  // if the HID is Mouse
        ...
        if (USBH_HID_GetRawData(phost, hid_raw_data) == USBH_OK) {
            // Check for your specific mouse (replace with actual VID/PID)
            bool is_specific_mouse = (phost->device.DevDesc.idVendor == 0x3710 &&
                (phost->device.DevDesc.idProduct == 0x5405 ||
                    phost->device.DevDesc.idProduct == 0x7402));

            if (is_specific_mouse && xlat_mode == XLAT_MODE_CLICK) {
                // For your mouse: report ID at byte 0, buttons at byte 1
                if (hid_raw_data[0] != 0x01) {
                    goto out;
                }
                static uint8_t prev_button = 0;
                uint8_t button = hid_raw_data[1]; // Hardcode button byte
                ...
                // Standard processing for other mice
                    ...
                    for (size_t idx = start_idx; idx < start_idx + length; idx++) {
                        if (hid_raw_data[idx]) {
                            ...
                            printf("Motion: X=0x%02x, Y=0x%02x @ %lu\n",
                                hid_raw_data[x_location.byte_offset],
                                hid_raw_data[y_location.byte_offset],
                                hevt->timestamp);
                            calculate_gpio_to_usb_time();
                            break;
                        }
